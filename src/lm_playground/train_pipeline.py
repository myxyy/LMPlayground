"""Pipeline-parallel training entry point for QGRUModel.

Usage:
  # Single GPU (falls back to regular DDP-style training)
  ...
  # 4-stage pipeline
  uv run torchrun --standalone --nproc_per_node=4 src/lm_playground/train_pipeline.py experiment=qgru
"""
...
from lm_playground.model.qgru import QGRUConfig, QGRUModel, QGRUPipelineStage


# ---------------------------------------------------------------------------
# Single-GPU trainer (no pipeline, lightweight DDP wrapper removed)
# ---------------------------------------------------------------------------
...
class SingleGPUTrainer:
    ...
    def save_checkpoint(self):
        d = self._ckpt_dir()
        # remove previous checkpoint for current epoch
...
class PipelineTrainer:
    def __init__(
        ...
        stage_infos = QGRUPipelineStage.split_config(config.num_layers, self.world_size)
        self.stage_info = stage_infos[self.rank]
        self.stage_module = QGRUPipelineStage(config, **self.stage_info).to(self.device)
    ...
    def save_checkpoint(self):
        d = self._ckpt_dir()
        # Each rank saves its own stage checkpoint
        path = os.path.join(
            d,
    ...
    def save_weight(self):
        ...
        if "_hidden_init_parts" in full_sd:
            ...
            full_sd["_hidden_init"] = torch.cat(
                [parts[k] for k in sorted(parts.keys())], dim=0
            )
        path = os.path.join(d, f"{self.model_name}.pth")
    ...
    def train(self):
        ...
        # All ranks use identical sampler so they see the same batches
...
def _build_datasets(cfg, tokenizer):
...
def main(cfg):
    ...
    train_dataset, validation_dataset = _build_datasets(cfg, tokenizer)

    partial_config = instantiate(cfg.model.config)
    config: QGRUConfig = partial_config(vocab_size=tokenizer.vocab_size)

    if rank == 0:
        print(f"World size: {world_size}")
        print(f"Model: {config.num_layers} layers, dim={config.dim}, dim_hidden={config.dim_hidden}")

    if world_size == 1:
        # ---- Single GPU: no pipeline ----
        model = QGRUModel(config)
        if rank == 0:
